...
type Division = { id: number | string; name: string; slug: string; description?: string | null };
type PageProps = { division: Division };
...
export default function Edit() {
  ...
  const onSubmit: React.FormEventHandler = (e) => {
    e.preventDefault();
    put(wf.update.url(toParam(d.id)), { preserveScroll: true });
  };

  const isIndex = url.startsWith("/admin/divisions") && !/\/create|\/edit/.test(url);
  const isCreateOrEdit = /\/divisions\/\d+\/edit/.test(url) || /\/divisions\/create/.test(url);
  ...
  return (
    <AppLayout>
      ...
      <PageHero
        ...
        title="Edit Division"
        description="Perbarui informasi divisi."
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setData("name", v);
                setData("slug", slugifyDot(v)); // selalu sinkron, slug auto
              }}
