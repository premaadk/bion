...
export default function Create() {
  ...
  const isIndex =
    url.startsWith("/admin/divisions") && !/\/create|\/edit/.test(url);
  const isCreateOrEdit =
    /\/divisions\/create/.test(url) || /\/divisions\/\d+\/edit/.test(url);
  ...
  return (
    <AppLayout>
      ...
      <PageHero
        ...
        description="Tambahkan divisi baru untuk mengelompokkan struktur organisasi."
        gradient={{
          from: "#203b8a",
          via: "#2a4db3",
          to: "#3560dc",
      ...
      <div className="container mx-auto max-w-7xl px-4 pt-6 pb-[calc(env(safe-area-inset-bottom)+96px)] md:pb-8">
        ...
        <form
          onSubmit={onSubmit}
          className="space-y-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm md:p-6 max-w-7xl"
        >
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setData("name", v);
                setData("slug", slugifyDot(v)); // ← selalu sinkron & otomatis
              }}
