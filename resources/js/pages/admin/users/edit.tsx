...
type WFUsers = {
  index: { url: () => string };
  create: { url: () => string };
  update: { url: (p: { user: number }) => string };
};
...
export default function Edit() {
  ...
  const { data, setData, put, processing, errors, reset } = useForm<FormData>({
    ...
    role: p.currentRole ?? "",
    direct_permissions: [...p.directPermissions],
    rubrik_id: p.user.rubrik_id,
    division_id: p.user.division_id,
  });

  // simpan "original rubrik" dari database untuk dipulihkan ketika role kembali mewajibkan rubrik
  const initialRubrikIdRef = useRef<number | null>(p.user.rubrik_id ?? null);

  useEffect(() => () => reset("password", "password_confirmation"), []); // bersih saat unmount

  /* ====== Aturan Rubrik vs Role ====== */
  const roleName = (data.role || "").trim().toLowerCase();
  const roleForbidsRubrik = roleName === "super admin" || roleName === "author";
  const roleNeedsRubrik =
    roleName === "admin rubrik" || roleName === "editor rubrik";
  const rubrikDisabled = !data.role || roleForbidsRubrik;

  // Saat role berubah:
  // - jika role kosong / melarang → kosongkan rubrik (tanpa mengubah nilai original di ref)
  // - jika role mewajibkan → jika rubrik kosong, pulihkan dari nilai awal database
  useEffect(() => {
    if (!data.role || roleForbidsRubrik) {
      if (data.rubrik_id !== null) setData("rubrik_id", null);
      return;
    }
    if (roleNeedsRubrik && (data.rubrik_id === null || data.rubrik_id === undefined)) {
      setData("rubrik_id", initialRubrikIdRef.current);
    }
  ...
  const rubrikPlaceholder = !data.role
    ? "— Pilih role terlebih dulu"
    : roleForbidsRubrik
    ? "— Tidak boleh memilih (Super Admin/Author)"
    : "— Pilih rubrik —";
  const showRubrikRequiredHint = roleNeedsRubrik && !rubrikDisabled && !data.rubrik_id;

  const submit: React.FormEventHandler = (e) => {
    e.preventDefault();
    // blokir submit jika role mewajibkan rubrik tapi belum dipilih
    if (roleNeedsRubrik && !data.rubrik_id) {
      rubrikRef.current?.focus();
      return;
    }
    put(wf.update.url({ user: Number(p.user.id) }), { preserveScroll: true });
  };

  // Stats (Hero)
  const totalRoles = p.roles.length;
  const totalPerms = p.permissions.length;
  ...
  }, [p.permissions, permQuery]);

  const togglePermission = (name: string) => {
    setData(
      "direct_permissions",
      data.direct_permissions.includes(name)
        ? data.direct_permissions.filter((n) => n !== name)
  ...
  return (
    <AppLayout>
      <Head title={`Edit User – ${p.user.name}`} />

      <PageHero
        badge={{ text: "Users", icon: <UsersIcon className="h-4 w-4" /> }}
        title={`Edit User: ${p.user.name}`}
        description="Perbarui profil pengguna, peran, divisi/rubrik, dan izin langsung."
        ...
        stats={[
          { value: 1, label: "Form", icon: <UserRound className="h-5 w-5" /> },
          { value: totalRoles, label: "Roles", icon: <UserRound className="h-5 w-5" /> },
      ...
      <div className="container mx-auto max-w-7xl px-4 pt-6 pb-[calc(env(safe-area-inset-bottom)+96px)] md:pb-8">
        ...
        <div className="md:hidden mb-6">
          <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-gray-200 shadow-sm">
            ...
            <Link
              href="#"
              className="flex flex-col items-center justify-center gap-1 border-l py-3 bg-[#1f3b8a] text-white"
              aria-current="page"
            >
        ...
        <div className="hidden md:flex mb-6 gap-2">
          ...
          <span
            aria-current="page"
            className="inline-flex items-center gap-2 rounded-2xl border px-5 py-3 text-sm font-semibold bg-[#1f3b8a] text-white border-[#1f3b8a]"
          >
            <UserPlus className="h-5 w-5" />
            Edit User
          </span>
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                <label className="block text-sm font-medium">
                  Rubrik {roleNeedsRubrik && !rubrikDisabled && <span className="text-red-600">*</span>}
                </label>
                {roleForbidsRubrik && (
              ...
              <div className="relative">
                ...
                <select
                  ref={rubrikRef}
                  className={[
                    "w-full rounded-lg border py-2 pl-9 pr-3 text-sm focus:ring-2",
                    rubrikDisabled
                      ? "bg-gray-100 text-gray-500 border-gray-200 cursor-not-allowed"
                      : "bg-gray-50 border-gray-200 focus:bg-white focus:border-gray-300 focus:ring-indigo-500/20",
                  ...
                  required={roleNeedsRubrik && !rubrikDisabled}
                >
