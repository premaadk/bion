...
type WFRubriks = {
  index: { url: () => string };
  create: { url: () => string };
  edit: { url: (p: { rubrik: number } | { id: number }) => string };
  destroy: { url: (p: { rubrik: number } | { id: number }) => string };
};
const wf = rubriksRoutes as unknown as WFRubriks;

const toParam = (id: Id) => ({ rubrik: Number(id) }) as any;

type PageProps = { rubriks: Rubrik[] };
...
export default function Index() {
  ...
  const items = (props as PageProps).rubriks ?? [];

  // Stats
  const totalRubriks = items.length;
  ...
  return (
    <AppLayout>
      <Head title="Management Rubriks" />

      <PageHero
        badge={{ text: "Rubriks", icon: <Layers className="h-4 w-4" /> }}
        title="Manajemen Rubrik"
        description="Kelola daftar rubrik/kategori editorial untuk konten Anda."
        ...
        stats={[
          { value: totalRubriks, label: "Total Rubriks", icon: <Layers className="h-5 w-5" /> },
        ...
        <div className="hidden overflow-hidden rounded-xl border border-gray-200 bg-white md:block">
          ...
          {filtered.length === 0 && (
            <div className="py-14 text-center">
              ...
              <p className="text-gray-500">Tidak ada rubrik.</p>
            </div>
          )}
        </div>
              ...
              <div key={r.id} className="rounded-xl border border-gray-200 bg-white p-4">
                <div className="mb-3 flex items-start justify-between">
                  ...
                  <div className="ml-2 flex items-center gap-2">
                    <button onClick={() => onEdit(r.id)} className="rounded p-2 text-gray-500 hover:text-indigo-600">
                      <Pencil className="h-5 w-5" />
                    </button>
                    <button onClick={() => onDelete(r.id)} className="rounded p-2 text-gray-500 hover:text-red-600">
                      <Trash2 className="h-5 w-5" />
                    </button>
                  </div>
                </div>
                {r.description && <p className="mb-3 text-sm text-gray-600">{r.description}</p>}
                <div className="border-t border-gray-100 pt-2 text-xs text-gray-500">Created: {fmtDate(r.created_at)}</div>
              </div>
            ))
          )}
        </div>
